<!-- Jquery Min JS -->
<script src="{{url('assets/frontend/js/jquery.min.js')}}"></script>
<!-- Bootstrap Min JS -->
<script src="{{url('assets/frontend/js/bootstrap.bundle.min.js')}}"></script>
<!-- Owl Carousel Min JS -->
<script src="{{url('assets/frontend/js/owl.carousel.min.js')}}"></script>
<!-- Nice Select Min JS -->
<script src="{{url('assets/frontend/js/jquery.nice-select.min.js')}}"></script>
<!-- Wow Min JS -->
<script src="{{url('assets/frontend/js/wow.min.js')}}"></script>
<!-- Meanmenu JS -->
<script src="{{url('assets/frontend/js/meanmenu.js')}}"></script>
<!-- Jquery Ui Min JS -->
<script src="{{url('assets/frontend/js/jquery-ui.min.js')}}"></script>
<!-- slick Min JS -->
<script src="{{url('assets/frontend/js/slick.min.js')}}"></script>
<!-- Ajaxchimp Min JS -->
<script src="{{url('assets/frontend/js/jquery.ajaxchimp.min.js')}}"></script>
<!-- Form Validator Min JS -->
<script src="{{url('assets/frontend/js/form-validator.min.js')}}"></script>
<!-- Contact Form JS -->
<script src="{{url('assets/frontend/js/contact-form-script.js')}}"></script>
<!-- Mixitup Min JS -->
<script src="{{url('assets/frontend/js/mixitup.min.js')}}"></script>

<!-- Custom JS -->
<script src="{{url('assets/frontend/js/custom.js')}}"></script>
@yield('scripts')
